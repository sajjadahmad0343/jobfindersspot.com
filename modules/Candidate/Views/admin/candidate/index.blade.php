        ...
        <div class="filter-div d-flex justify-content-between ">
            ...
            <div class="col-left">
                <form method="get" action="{{url('/admin/module/candidate/')}} " class="filter-form filter-form-right d-flex justify-content-end flex-column flex-sm-row" role="search">
                    ...
                    <select name="allow_search" class="form-control dddd">
                        {{-- <option value="">{{ __('-- Allow Search --')}} </option> --}}
                        <option value="publish">{{ __('-- Allow Search --')}} </option>
                                ...
                                @if($rows->total() > 0)
                                    @foreach($rows as $row)
                                        ...
                                        <tr>
                                            ...
                                            <td><span class="badge badge-{{ $row->candidate!=null?($row->candidate->allow_search == 'publish' ? 'active' : 'warning'):'danger' }}">{{ $row->candidate!=null?($row->candidate->allow_search == 'publish' ? __('Publish') : __('Hide')):'' }}</span></td>
                                            <td>
